...
class StockScreener:
    ...
    def fetch_stock_data(self, symbol, period='180', freq='D', use_tushare_only=False):
        """
        获取单个股票的历史数据
        ...
        try:
            ...
            if freq != 'D':
                ...
                # 如果所有真实数据源都失败，返回None表示获取失败
                print(f"❌ 所有数据源均不可用，无法获取 {symbol} 的数据")
                return None
        except Exception as e:
