#!/usr/bin/env python3
"""
Corrected analysis script for stock 601777 based on the past 360 days of data.
Based on online verification, 601777 is 国投资本 (SDIC Capital).
"""
...
def analyze_stock():
    """Analyze stock 601777 (国投资本) based on the past 360 days of data."""

    print("Starting analysis for stock: 601777 (国投资本 - SDIC Capital)")
    ...
    # Stock symbol - using correct format for国投资本
    symbols = ["601777.SH"]  # Tushare format for Shanghai stocks

    print(f"\nFetching historical data for {symbols[0]} (国投资本)...")
    ...
    if symbols[0] in data and not data[symbols[0]].empty:
        ...
        print(f"Current RSI: {df_sorted['rsi'].iloc[-1]:.2f}")

        # Determine if stock is overbought/oversold
        rsi_val = df_sorted['rsi'].iloc[-1]
        if rsi_val > 70:
            print("RSI indicates: Overbought (potentially overvalued)")
        elif rsi_val < 30:
            print("RSI indicates: Oversold (potentially undervalued)")
        else:
            print("RSI indicates: Neutral")

        # Export to CSV for further analysis if needed
        output_file = f"601777_analysis_{datetime.now().strftime('%Y%m%d')}.csv"
