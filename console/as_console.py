"""
Unified Interactive Console for A-Share Market Analysis
Provides access to all system functions through interactive commands
"""
import sys
import os
import pandas as pd
        ...
        try:
            ...
            if latest_signals:
                ...
                for i, signal in enumerate(latest_signals, 1):
                    signal_type = "🟢 买入" if signal['signal_type'] == 'BUY' else "🔴 卖出" if signal['signal_type'] == 'SELL' else "⏸️  持有"
                    print(f"  {i}. {signal_type} - {signal['symbol']} ({signal['strategy']}): {signal['reason']}")
            else:
                print("\n✅ 暂无最新信号")
        except Exception as e:
            print(f"❌ 获取信号出错: {e}")
