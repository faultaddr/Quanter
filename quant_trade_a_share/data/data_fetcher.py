...
# Conditionally import yfinance to handle compatibility issues
try:
...
class DataFetcher:
    def __init__(self):
        ...
        self.ashare_fetcher = AshareDataFetcher()  # Add Ashare fetcher
    ...
    def fetch(self, symbol, start_date, end_date, source="eastmoney"):
        ...
        Args:
            ...
            source (str): Data source ('eastmoney', 'tushare', 'baostock', 'yahoo', 'ashare')
        ...
        if source == "eastmoney":
            ...
            if YFINANCE_AVAILABLE:
                ...
                print("Yahoo Finance not available, using EastMoney as fallback")
                return self._fetch_eastmoney(symbol, start_date, end_date)
        elif source == "ashare":
            return self._fetch_ashare(symbol, start_date, end_date)
        else:
    ...
    def _fetch_ashare(self, symbol, start_date, end_date):
        ...
        try:
            ...
            # Make sure we have enough days to cover the requested range
            requested_days = max(days, 30)

            # Use Ashare fetcher to get stock data
            df = self.ashare_fetcher.fetch_stock_data(symbol, days=requested_days)
            ...
            if df is not None and not df.empty:
                ...
                if missing_cols:
                    print(f"Warning: Missing columns in Ashare data: {missing_cols}")
    ...
    def _fetch_yahoo(self, symbol, start_date, end_date):
        ...
        if not YFINANCE_AVAILABLE:
            print("Error: Yahoo Finance source not available (yfinance not installed or incompatible)")
            return pd.DataFrame()
