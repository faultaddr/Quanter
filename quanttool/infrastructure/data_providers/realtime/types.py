...
参考 ZhuLinsen/daily_stock_analysis 的 realtime_types.py 设计
...
使用方式：
- 所有 Provider 的 get_realtime_quote() 统一返回 RealtimeQuote
...
class RealtimeQuote:
    ...
    # === 五档买卖 ===
    bid_prices: Optional[List[float]] = None  # 买一~买五价
    bid_volumes: Optional[List[int]] = None   # 买一~买五量
    ask_prices: Optional[List[float]] = None  # 卖一~卖五价
    ask_volumes: Optional[List[int]] = None   # 卖一~卖五量
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        for f in optional_fields:
            ...
            if val is not None:
                if f == 'timestamp':
                    result[f] = val.isoformat() if isinstance(val, datetime) else val
                else:
                    result[f] = val
        return result
    ...
    def has_five_level(self) -> bool:
        """检查是否有五档数据"""
        return self.bid_prices is not None and len(self.bid_prices) > 0
...
def normalize_symbol(symbol: str) -> Tuple[str, int]:
    """
    标准化股票代码，返回 (纯代码, 市场代码)
    ...
    # 指数代码特殊处理
    # 深证指数: 399xxx - 深圳
    # 上证指数: 000001-000999, 9xxxxx - 上海
    if code.startswith('399'):
        return code, 0  # 深圳（深证成指、创业板指等）

    if code.startswith(('000', '880', '999')):
        # 上证指数代码（000001上证指数、000300沪深300等）
        return code, 1  # 上海
