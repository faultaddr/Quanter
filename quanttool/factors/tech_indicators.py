"""
Technical analysis indicators module based on MyTT indicators
Implementation of various technical analysis functions for stock analysis
"""
import numpy as np
import pandas as pd
...
def FILTER(S, N):
    ...
    for i in range(len(S_copy)):
        if S_copy[i]:
            S_copy[i+1:min(i+1+N, len(S_copy))] = 0
    return S_copy                           # 例：FILTER(C==H,5) 涨停后，后5天不再发出信号
...
def DMI(CLOSE, HIGH, LOW, M1=14, M2=6):
    """动向指标：结果和同花顺，通达信完全一致"""
    TR = SUM(MAX(MAX(HIGH - LOW, ABS(REF(CLOSE, 1) - HIGH)), ABS(REF(CLOSE, 1) - LOW)), M1)
    HD = HIGH - REF(HIGH, 1)
    LD = REF(LOW, 1) - LOW
