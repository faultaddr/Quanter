...
# GBDT 模型列表
GBDT_MODELS = ['lgb', 'xgboost', 'catboost']
...
def main():
    ...
    print("\n📊 模型收益排名:")
    print("-" * 80)
    print(f"{'排名':<4} {'模型':<15} {'年化收益':>10} {'夏普':>8} {'胜率':>8} {'交易次数':>8} {'回撤':>8}")
    print("-" * 80)

    for i, r in enumerate(successful_results, 1):
        print(f"{i:<4} {r['model_type'].upper():<15} {r['annual_return']*100:>9.2f}% "
              f"{r.get('avg_sharpe', 0):>8.2f} {r.get('avg_win_rate', 0)*100:>7.1f}% "
              f"{r.get('total_trades', 0):>8} {r.get('avg_drawdown', 0)*100:>7.2f}%")
    ...
    if successful_results:
        ...
        print(f"   总交易次数: {best.get('total_trades', 0)}")

        # 各股票详情
        print(f"\n📈 各股票详细结果:")
        for r in best.get('individual_results', []):
            print(f"   {r['symbol']}: 年化 {r['annual_return']*100:.2f}%, "
                  f"夏普 {r['sharpe_ratio']:.2f}, 胜率 {r['win_rate']*100:.1f}%")
